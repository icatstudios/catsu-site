  ...
  return (
    <main className="cosmic-bg relative flex min-h-screen flex-col overflow-hidden">
      ...
      <section className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 pb-24 text-center">
        {/* logo with halo */}
        <div className="rise rise-1 relative mb-9 flex items-center justify-center">
          <span
            aria-hidden
            className="animate-halo absolute left-1/2 top-1/2 h-64 w-64 rounded-full"
            style={{
              background:
                "radial-gradient(circle, rgba(106,63,208,0.55), rgba(245,185,66,0.12) 45%, transparent 70%)",
            }}
          />
          <div className="animate-float logo-glow relative">
            <Image
              ...
              height={190}
              priority
              className="h-40 w-40 rounded-[28px] ring-1 ring-[var(--hairline)] sm:h-48 sm:w-48"
            />
          </div>
        </div>
        ...
        {/* headline */}
        <h1 className="rise rise-2 gold-text text-glow-gold max-w-4xl text-balance text-4xl font-extrabold leading-[1.05] sm:text-6xl md:text-7xl">
          {t("title")}
        </h1>
